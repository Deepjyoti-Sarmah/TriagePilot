...
- [x] Methodology fix: synthetic rows get inline `title`/`body` so direct-path evals are fair; re-run sweep
- [x] Re-sweep 2026-09-19 (`sweep-002.json`, DeepSeek free, same seed): **0.800 (8/10) vs gate 0.8 → PASS (boundary)**. 2 rows 503'd (free-pool instability, excluded as unscored). Misses: VibeCode PR#9 → spam (terse title, empty body — model played safe); syn-dup-install → bug not duplicate (single-report duplicate detection needs memory/tools = full-agent path, structural ceiling of classify-only).
